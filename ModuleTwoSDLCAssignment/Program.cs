...
internal class Parser
{
    ...
    static void InitializeWindows()
    {
        ...
        WinApp.Run(MainWindow = new UserControl1()); 
    }
    ...
    public static void countOccurences(List<string> regexList) //Pass the List generated in the readAndParseFile into this method.
    {
        ...
        var numOfMatches = matches.Count();
    ...
    protected static void readAndParseFile()
    {
        ...
        matches = regex.Matches(File.ReadAllText(outputToLowercase)).OfType<Match>().Select(m => m.Value); //Probably redundant, but output the text after reading it to lowercase, just as a safety valve. Select each value output by the Regex expression and output them to a new form.
